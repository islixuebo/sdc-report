...
read -p "Jira 服务器地址（默认: https://idisplayvision.com/jira/rest/api/2/search）: " input_jira_url
JIRA_URL="${input_jira_url:-https://idisplayvision.com/jira/rest/api/2/search}"

read -p "Jira API Token（必填）: " input_jira_token
...
read -p "JQL 查询条件（默认: project = SDCDN AND issuetype in (任务, 改进) AND fixVersion = EMPTY）: " input_jql
JQL="${input_jql:-project = SDCDN AND issuetype in (任务, 改进) AND fixVersion = EMPTY}"
